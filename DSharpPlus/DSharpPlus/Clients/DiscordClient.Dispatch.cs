...
namespace DSharpPlus
{
    public sealed partial class DiscordClient
    {
        ...
        internal async Task OnMessageCreateEventAsync(DiscordMessage message, TransportUser author)
        {
            ...
            if(!usr.IsBot && guild != null && message.Channel != null)
            {
                DisabledChannels dc = new DisabledChannels(guild.Id.ToString(CultureInfo.InvariantCulture), message.Channel?.Id.ToString(CultureInfo.InvariantCulture));
                await dc.Select();
                if (dc.isDisabled)
